...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    private static final String CREATE_TABLE="CREATE TABLE "+TABLE_NAME+" ("+ID+" INTEGER PRIMARY KEY AUTOINCREMENT, "+NAME+" VARCHAR(50), "+DESCRIPTION+" TEXT, "+PRIORITY+" TEXT, "+HOURS+" INTEGER, "+MINUTES+" INTEGER,"+DAYS+" INTEGER ,"+MONTHS+" INTEGER,"+YEARS+" INTEGER,"+COMPLETED+" INTEGER,"+NOTIF+" INTEGER);";
    ...
    public long saveData(String name,String description,String priority,int hours,int minutes,int days,int months,int years,int completed,int notif)
    {
    ...
    public Cursor showDataForDate(int choice0,int choice1,String day,String month,String year)
    {
        SQLiteDatabase sqLiteDatabase=this.getWritableDatabase();
        Cursor cursor=sqLiteDatabase.rawQuery("SELECT * FROM "+TABLE_NAME+" WHERE "+DAYS+" = "+day+" AND "+MONTHS+" = "+month+" AND "+YEARS+" = "+year+" AND ("+COMPLETED+" = "+choice0+" OR "+COMPLETED+" = "+choice1+" )",null);
        return cursor;
    }
    ...
    public Cursor showPinnedTaskDec(String year)
    {
        SQLiteDatabase sqLiteDatabase=this.getWritableDatabase();
        Cursor cursor=sqLiteDatabase.rawQuery("SELECT * FROM "+TABLE_NAME+" WHERE "+YEARS+" = "+year+" ORDER BY "+ID+" DESC",null);
        return cursor;
    }
}
